<div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-1">
                        ...
                        @if (!empty($conversation->agent_class))
                            ...
                            <x-ai-analyzer::badge :label="class_basename($conversation->agent_class)" color="blue" />
                        @endif
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <form method="POST" action="{{ route('analyzer.export.pest', $conversationId) }}" class="inline">
                        @csrf
                        <button type="submit" class="analyzer-btn-secondary text-xs px-2.5 py-1.5" title="Export to Pest test">
                            Pest
                        </button>
                    </form>
                    <form method="POST" action="{{ route('analyzer.export.json', $conversationId) }}" class="inline">
                        @csrf
                        <button type="submit" class="analyzer-btn-secondary text-xs px-2.5 py-1.5" title="Export to JSONL">
                            JSONL
                        </button>
                    </form>
                    <button
                        wire:click="toggleRawPayload"
                        class="analyzer-btn-secondary"
                    >
    ...
    {{-- Messages Timeline --}}
    @if ($messages->isEmpty())
        <x-ai-analyzer::empty-state title="No messages" description="This conversation has no recorded messages." />
    @else
        <div class="space-y-4">
    ...
    @if ($runs->isNotEmpty())
        <x-ai-analyzer::card title="Related Runs" padding="p-0">
            <div class="divide-y divide-gray-200/60 dark:divide-white/8">
                @foreach ($runs as $run)
                    <a href="{{ route('analyzer.runs.show', $run) }}" class="flex items-center justify-between gap-4 px-4 py-3 hover:bg-gray-50/80 dark:hover:bg-white/5">
                        <div>
