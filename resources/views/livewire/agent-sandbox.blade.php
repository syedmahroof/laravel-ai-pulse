    ...
    <div class="flex-1 overflow-y-auto space-y-3 mb-4 pr-2" id="sandbox-messages">
        @if (empty($history))
            <div wire:loading.remove wire:target="send" class="flex items-center justify-center h-full">
                <x-ai-pulse::empty-state title="Start a conversation"
    ...
    <div class="flex-shrink-0">
        ...
        <form wire:submit.prevent="send"
            ...
            <textarea
                ...
                class="pulse-input flex-1 disabled:opacity-50 resize-none"
            ></textarea>
            <button
                type="submit"
                wire:loading.attr="disabled" wire:target="send"
                :disabled="$wire.prompt === '' || ($wire.simulationMode === 'pending' && $wire.needsInput) || $wire.simulationMode === 'unavailable'"
                class="pulse-btn-primary disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center justify-center gap-1.5 flex-shrink-0 w-[115px]"
            >
            ...
            @if (!empty($history))
                <button
                    wire:click="clear"
                    type="button"
                    class="pulse-btn-secondary whitespace-nowrap"
                >
                    Clear
                </button>
            @endif
