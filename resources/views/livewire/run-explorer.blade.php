<div>
    ...
    <div class="run-filters mb-5">
        ...
        <select wire:model.live="operation" class="analyzer-input run-select">
        ...
        <select wire:model.live="status" class="analyzer-input run-select">
        ...
        <select wire:model.live="conversationState" class="analyzer-input run-select">
            <option value="">Any Conv</option>
            <option value="linked">Linked</option>
            <option value="unlinked">Unlinked</option>
        </select>

        <select wire:model.live="dateRange" class="analyzer-input run-select">
        ...
        <select wire:model.live="provider" class="analyzer-input run-select">
        ...
        <select wire:model.live="model" class="analyzer-input run-select">
        ...
        <select wire:model.live="agentClass" class="analyzer-input run-select">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/4">
                    @forelse ($runs as $run)
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-4 py-3">
                                <a href="{{ route('analyzer.runs.show', $run) }}"
                            ...
                            <td class="px-4 py-3 hidden md:table-cell">
                                <x-ai-analyzer::badge :label="$run->status" :color="$run->status === 'completed' ? 'green' : ($run->status === 'failed' ? 'red' : 'yellow')" />
                            </td>
                            ...
                            <td class="px-4 py-3 {{ $run->missing_pricing ? 'text-amber-600 dark:text-amber-300' : 'text-gray-500 dark:text-gray-400' }}">
                                <span>{{ config('ai-analyzer.currency_symbol', '$') }}{{ number_format((float) $run->cost, 6) }}</span>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    @if ($run->conversation_id)
                                        <a href="{{ route('analyzer.conversations.show', $run->conversation_id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-12">
                                <x-ai-analyzer::empty-state title="No runs found"
