    ...
    <x-ai-pulse::card>
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex gap-2">
                <select wire:model.live="period" class="pulse-input">
                    @foreach($periods as $key => $label)
                    <option value="{{ $key }}">{{ $label }}</option>
                    @endforeach
                </select>
                <select wire:model.live="groupBy" class="pulse-input">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
            <x-ai-pulse::stat label="Total Cost" value="{{ $currencySymbol }}{{ number_format($totalCost, 4) }}" />
            <x-ai-pulse::stat label="Conversations" value="{{ number_format($stats['total_conversations']) }}" />
            <x-ai-pulse::stat label="Input Tokens" value="{{ number_format($stats['input_tokens']) }}" />
            <x-ai-pulse::stat label="Output Tokens" value="{{ number_format($stats['output_tokens']) }}" />
        </div>
        ...
        @else
        <x-ai-pulse::empty-state message="No data for the selected period." />
        @endif
    </x-ai-pulse::card>
</div>
