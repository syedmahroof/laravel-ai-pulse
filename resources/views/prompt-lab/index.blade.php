<x-ai-analyzer::layout>
    ...
    <div class="space-y-6">
        ...
        <livewire:ai-analyzer.prompt-lab />
                    ...
                    <table class="analyzer-table w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/4">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <x-ai-analyzer::badge :variant="$session->status === 'completed' ? 'success' : 'warning'">
                                        {{ ucfirst($session->status) }}
                                    </x-ai-analyzer::badge>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $session->created_at->format('M d, Y H:i') }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('analyzer.prompt-lab.show', $session->id) }}" class="text-orbit-500 hover:text-orbit-600 dark:text-orbit-400 dark:hover:text-orbit-300 text-sm font-medium">View</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
