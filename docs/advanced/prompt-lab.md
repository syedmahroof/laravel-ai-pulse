...
Navigate to `/ai-pulse/prompt-lab`.
...
Click **"Run Comparison"**. AI Pulse sends the prompt to all configured slots simultaneously and displays results as they arrive.
...
AI Pulse automatically tags the best performers:
...
Every comparison is saved as a session. Browse past sessions at `/ai-pulse/prompt-lab`.
...
The `PulsePromptLabAgent` is a lightweight agent implementation used specifically for the Prompt Lab:
...
Adjust Prompt Lab settings in `config/ai-pulse.php`:
...
Then edit:
- `resources/views/vendor/ai-pulse/prompt-lab/index.blade.php` — Session list
- `resources/views/vendor/ai-pulse/prompt-lab/compare.blade.php` — Comparison interface
- `resources/views/vendor/ai-pulse/prompt-lab/show.blade.php` — Session detail
