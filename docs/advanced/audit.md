...
AI Analyzer's Audit & Compliance section provides security scanning, data retention management, and access logging to help you maintain control over your AI operations.

## Access

Navigate to `/ai-analyzer/audit`.
...
Manage how long conversation data is kept. AI Analyzer provides both dry-run previews and actual purging.

### Retention Policy

The default retention period is 90 days, configurable in `config/ai-analyzer.php`:
...
AI Analyzer automatically cleans up sandbox sessions (conversations with `user_id = 0`) that are older than 24 hours. This prevents the sandbox from filling up your database.
...
AI Analyzer logs all dashboard access attempts via the `Authorize` middleware. Review the audit log to see:
...
Then edit:
- `resources/views/vendor/ai-analyzer/audit/index.blade.php` — Audit layout
- `resources/views/vendor/ai-analyzer/livewire/audit-dashboard.blade.php` — Audit dashboard component
