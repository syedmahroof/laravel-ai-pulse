...
AI Analyzer follows [Semantic Versioning](https://semver.org/). Upgrades within the same major version should be seamless.
...
2. **Run Composer update:**
   ```bash
   composer update syedmahroof/ai-analyzer
   ```
...
After every update, **always republish assets** with `--force`. AI Analyzer ships compiled CSS and favicon files that may change between releases. Skipping this step can lead to broken styling or missing icons.
:::

## Version Compatibility

| AI Analyzer Version | Laravel | PHP | Laravel AI SDK |
|:---|:---|:---|:---|
...
2. **New `ai-analyzer:install` command** — A new one-command installer is available for fresh installs:
   ```bash
...
1. **Config file changes** — The config structure was reorganized. Compare your published `config/ai-analyzer.php` with the latest version and merge any new keys.

2. **New migrations** — Run migrations to create new AI Analyzer tables:
   ```bash
   php artisan migrate
   ```

3. **Livewire component tags** — If you've overridden views that reference Livewire components, note that component names are registered with the `ai-analyzer.` prefix:
   ```blade
...
- Watch the [GitHub repository](https://github.com/syedmahroof/ai-analyzer) for releases
