...
Complete reference for all AI Pulse dashboard routes.

## Route Configuration

All routes are prefixed with the configured path (default: `ai-pulse`) and protected by the `Authorize` middleware.
...
If you've configured a custom domain in `config/ai-pulse.php`:
...
```php
// Named routes
route('pulse.dashboard');           // /ai-pulse
route('pulse.conversations.show', ['id' => 'abc-123']); // /ai-pulse/conversations/abc-123
route('pulse.playground.show', ['agent' => 'App\AI\Agents\SupportAgent']);
route('pulse.traces.show', ['id' => 'abc-123']);
```

## Customizing Routes

To add custom routes to the AI Pulse route group, create a service provider that loads additional routes:
    ...
    public function boot(): void
    {
        Route::group([
            ...
            'middleware' => array_merge(
                ...
                [\Syedmahroof\AiPulse\Http\Middleware\Authorize::class]
            ),
        ], function () {
...
1. **Configured middleware** — From `config/ai-pulse.php` (`middleware` key)
2. **Authorize middleware** — Automatically appended; checks the `viewAiPulse` Gate
