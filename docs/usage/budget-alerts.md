...
Navigate to `/ai-pulse/usage/alerts`.
...
1. AI Pulse calculates current spending for the alert's period
...
1. Aggregating token usage for the period via `TokenAggregator` from both SDK conversations and `pulse_ai_runs`
...
- A link to the AI Pulse dashboard

AI Pulse ships with both HTML and plain-text email templates for budget alerts. You can test delivery for any alert directly from the dashboard using the **"Test Email"** action.
...
Then edit `resources/views/vendor/ai-pulse/livewire/budget-alerts.blade.php`.
...
Then edit:
- `resources/views/vendor/ai-pulse/emails/budget-exceeded.blade.php` — HTML email
- `resources/views/vendor/ai-pulse/emails/budget-exceeded-text.blade.php` — Plain-text email
