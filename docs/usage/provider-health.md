...
Navigate to `/ai-analyzer/usage/health`.

## Health Metrics

For each provider, AI Analyzer displays:
...
1. **`analyzer_ai_runs`** — One-off SDK calls with provider, model, latency, and status stored directly in typed columns
...
- **From runs**: Direct `latency_ms` column on `analyzer_ai_runs`
...
```php
use Syedmahroof\AiAnalyzer\Services\ProviderHealthChecker;
...
AI Analyzer includes a `ProviderDown` notification that can be integrated into your monitoring:

```php
use Syedmahroof\AiAnalyzer\Notifications\ProviderDown;
...
Then edit `resources/views/vendor/ai-analyzer/livewire/provider-health.blade.php`.
