...
The Pricing Matrix lets you define and manage per-model token pricing. AI Analyzer uses these rules to calculate costs across the dashboard, analytics, and budget alerts.

## Access

Navigate to `/ai-analyzer/usage/pricing`.
...
When calculating costs, AI Analyzer:
...
AI Analyzer does not ship with pre-configured pricing. We recommend adding rules for all models you use. You can find current pricing on your provider's documentation:
...
```php
use Syedmahroof\AiAnalyzer\Models\PricingRule;
...
// Calculate cost
$calculator = app(\Syedmahroof\AiAnalyzer\Services\CostCalculator::class);
$cost = $calculator->calculate('gpt-4', 156, 89);
```

## Multi-Currency Support

Each rule has its own currency. The dashboard displays costs in the currency symbol configured in `config/ai-analyzer.php`:
...
2. **Update rules when provider pricing changes** — AI Analyzer does not auto-update pricing
...
Then edit `resources/views/vendor/ai-analyzer/livewire/pricing-matrix.blade.php`.
