...
AI Pulse ships with compiled CSS assets. These are published as part of the `ai-pulse:install` command. To publish only the assets:
...
After publishing, edit `public/vendor/ai-pulse/css/orbit.css` directly.
...
AI Pulse uses Tailwind CSS. You can customize the appearance by modifying Tailwind classes in the published Blade views.

## Rebuilding from Source

If you want to modify the source CSS and rebuild:

1. Clone the AI Pulse repository
...
AI Pulse's Tailwind config is in `tailwind.config.js`:
...
AI Pulse supports both dark and light modes. The theme is toggled via a button in the navigation bar.
