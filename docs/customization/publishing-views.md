...
AI Analyzer's entire UI is built with Blade templates that you can override to match your needs. Publish the views and customize any aspect of the dashboard.
...
```
resources/views/vendor/ai-analyzer/
```
...
Livewire components handle the interactive parts of AI Analyzer. When you publish views, you get the Blade templates, but the PHP logic stays in the package.

To customize behavior, you can:

1. **Override the view** — Change the HTML structure
2. **Extend the component** — Create a custom Livewire component that extends AI Analyzer's
...
Livewire::component('ai-analyzer.thread-explorer', CustomThreadExplorer::class);
```
...
```bash
rm -rf resources/views/vendor/ai-analyzer
```

Or re-publish with the `--force` flag to overwrite your changes:

```bash
php artisan vendor:publish --tag=ai-analyzer-views --force
```

## Best Practices

1. **Keep changes minimal** — Only override what you need to change
2. **Watch for updates** — New AI Analyzer versions may add features to views; re-publish after major updates
