...
Navigate to `/ai-pulse/runs` for the Run Explorer, or `/ai-pulse/runs/{id}` for a specific run's detail view.

## How It Works

When SDK Observability is enabled, AI Pulse listens to Laravel AI SDK events and persists run metadata to the `pulse_ai_runs` table. This happens automatically — no code changes required in your application.
...
Control observability via `config/ai-pulse.php`:
...
| `store_runs` | Persist runs to `pulse_ai_runs`. Disable to keep event-driven budget checks without storing data |
...
AI Pulse merges analytics from two sources for complete coverage:

1. **`agent_conversation_messages`** — Multi-turn conversation data (primary source)
2. **`pulse_ai_runs`** — One-off SDK calls where `conversation_id` is `NULL`
...
Then edit:
- `resources/views/vendor/ai-pulse/runs/index.blade.php` — Run Explorer layout
- `resources/views/vendor/ai-pulse/runs/show.blade.php` — Run detail layout
- `resources/views/vendor/ai-pulse/livewire/run-explorer.blade.php` — Livewire filter component
