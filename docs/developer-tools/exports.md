...
AI Analyzer can export any conversation to two formats: Pest PHP tests and OpenAI JSONL fine-tuning format. Export from the Message Timeline or programmatically via the API.
...
|:---|:---|:---|
| POST | `/ai-analyzer/export/pest/{id}` | Download Pest test file |
| POST | `/ai-analyzer/export/json/{id}` | Download JSONL file |
