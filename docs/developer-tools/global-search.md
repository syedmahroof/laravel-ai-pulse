...
Global Search lets you find conversations, messages, and content across the entire AI Analyzer dataset. Search from any page using the search bar or programmatically via the API.

## Using Global Search

### From the UI

The search box is available in the AI Analyzer navigation bar. Press `Cmd+K` (Mac) or `Ctrl+K` (Windows/Linux) to focus the search input.
...
```php
use Syedmahroof\AiAnalyzer\Services\GlobalSearch;
