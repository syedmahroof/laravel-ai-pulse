...
class AgentRegistry implements AgentRegistryContract
{
     ...
     */
    private const CACHE_KEY_CLASSES = 'pulse:agent_registry:classes';

    /**
     * Cache key prefix for individual agent metadata.
     */
    private const CACHE_KEY_PREFIX_META = 'pulse:agent_registry:meta:';
    ...
    {
        $cacheTtl = PulseConfig::registryCacheTtl();
    ...
    private function discoverAgents(): array
    {
        ...
        foreach (PulseConfig::agentDirs() as $dir) {
            if (! is_dir($dir)) {
                continue;
            }
