...
class AiRunRecorder
{
    ...
    {
        if (! $this->shouldRecord('failover') || ! Schema::hasTable('analyzer_ai_runs')) {
            return;
        }
    ...
    {
        if (! Schema::hasTable('analyzer_ai_runs')) {
            return;
        }
    ...
    private function shouldRecord(string $operation): bool
    {
        ...
        if (! config('ai-analyzer.observability.store_runs', true)) {
            return false;
        }

        return ! in_array($operation, config('ai-analyzer.observability.excluded_operations', []), true);
    }
    ...
    {
        $captureText = config('ai-analyzer.observability.capture_text_payloads', true);
        $payload = [
    ...
    {
        return mb_substr($value, 0, (int) config('ai-analyzer.observability.max_payload_length', 10000));
    }
}
