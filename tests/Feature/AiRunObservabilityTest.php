...
it('records one-off agent prompts when observability is enabled', function () {
    ...
    expect($run)->not->toBeNull()
        ...
        ->and($run->payload['prompt'])->toBe('Explain AI Analyzer')
        ->and($run->payload['response'])->toBe('AI Analyzer observes SDK calls.');
});

it('does not record runs when observability is disabled', function () {
    config()->set('ai-analyzer.observability.enabled', false);
...
it('does not store runs when run storage is disabled', function () {
    config()->set('ai-analyzer.observability.store_runs', false);
...
it('respects payload capture settings and truncation', function () {
    config()->set('ai-analyzer.observability.max_payload_length', 5);
    ...
    AiRun::query()->delete();
    config()->set('ai-analyzer.observability.capture_text_payloads', false);
...
function makeRunFixtures(
    string $invocationId,
    string $promptText = 'Explain AI Analyzer',
    string $responseText = 'AI Analyzer observes SDK calls.'
): array {
